...
cmb_map_start = rotate_to_top(cmb_map_og*mask_ring, vec_lon, vec_lat)
...
for i in range(6):
	...
	for j in range(no_sims):
		...
		cmb_map_sim = rotate_to_top(cmb_map_sim, vec_lon, vec_lat)
		cmb_map_sim = hpt.rotate_map(cmb_map_sim, rot_vec, np.pi/12+i*rot_ang)
		for k in range(no_circles):
			rad_lag = 0
			strip_finder(cmb_map_sim, ang_rad[k], NSIDE)
			circle_a = load_file('strip_a', bins)
			circle_b = load_file('strip_b', bins)
			x_corr_sims_0[j,k] = match_circle_s(circle_a, circle_b, T_m, m_max, rad_lag)
